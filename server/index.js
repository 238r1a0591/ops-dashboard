...
const { authenticateToken, allowRoles } = require("./auth/middleware");
app.use(cors());
app.use(express.json());
...
app.get('/api/who', async (req, res) => {
  try {
    const data = await fetchWithCache('who',
      'https://ghoapi.azureedge.net/api/NCDMORT3070?$top=10', 86400);
    res.json({ data, lastUpdated: new Date().toISOString() });
  } catch (error) {
...
app.get('/api/sop-board', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to fetch SOP board data' });
  }
});
...
app.get('/api/clockify', async (req, res) => {
  try {
    const data = await fetchWithCache('clockify-workspaces',
...
app.get('/api/news', async (req, res) => {
  try {
    const data = await fetchWithCache('news-v3',
      `https://newsapi.org/v2/everything?q=india&language=en&sortBy=publishedAt&pageSize=5&apiKey=${process.env.NEWSAPI_KEY}`,
      1800);
    res.json({ data, lastUpdated: new Date().toISOString() });
  } catch (error) {
...
app.get('/api/check-triggers', async (req, res) => {
  try {
    ...
    const AQI_THRESHOLD = 80;
    if (currentAqi > AQI_THRESHOLD) {
      createActionIfNotExists(
        'aqi-hyderabad', 'WFH-Advisory', 'ops-team@company.com',
        `AQI is ${currentAqi}, exceeds threshold of ${AQI_THRESHOLD}`, 24
      );
    }
